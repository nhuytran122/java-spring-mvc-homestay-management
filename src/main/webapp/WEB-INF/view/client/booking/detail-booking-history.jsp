    ...
    <div class="container py-5">
        ...
        <div class="row g-4">
                ...
                <div class="card mb-4">
                    <c:choose>
                        <c:when test="${not empty room.thumbnail}">
                            <img src="/images/room/${room.thumbnail}" class="property-image">
                        </c:when>
                        <c:otherwise>
            ...
            <div class="col-lg-4">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <div>
                                <span class="badge ms-2 ${status == 'COMPLETED' ? 'bg-success' :
                                    status == 'CANCELLED' ? 'bg-danger' :
                                    status == 'CONFIRMED' ? 'bg-primary' : 'bg-info'}">
                                    ${status.displayName}
                                </span>

                                <c:if test="${status == 'CANCELLED' && not empty booking.payments}">
                ...
                <c:set var="bookingExtensions" value="${booking.bookingExtensions}"/>
                <c:if test="${not empty bookingExtensions}">
                    <div class="card mb-4">
                        <div class="card-header bg-white">
                            <h5 class="mb-0"><i class="bi bi-clock-history me-2"></i>Lịch sử gia hạn</h5>
                        </div>
                        <div class="card-body">
                            <c:forEach var="extension" items="${bookingExtensions}">
                                <div class="mb-3">
                ...
                <c:set var="bookingServices" value="${booking.bookingServices}"/>
                <c:if test="${not empty bookingServices}">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                                <c:if test="${not empty prepaidServices}">
                                    <h6 class="fw-bold mb-3">Dịch vụ trả trước</h6>
                                    <c:forEach var="bService" items="${prepaidServices}">
                                        <c:set var="serviceOfBService" value="${bService.service}"/>
                                            <div class="d-flex justify-content-between mb-2">
                                                <span>${serviceOfBService.serviceName}</span>
                                                <span class="fw-bold">
                                                    <fmt:formatNumber value="${serviceOfBService.price}" />đ x
                            ...
                            <c:if test="${not empty postpaidServices}">
                                <h6 class="fw-bold mb-3">Dịch vụ trả sau</h6>
                                <c:forEach var="bService" items="${postpaidServices}">
                                        ...
                                        <c:choose>
                                            <c:when test="${not empty bService.quantity}">
                                                <div class="d-flex justify-content-between mb-2 align-items-center">
                                                    <div>
                                                        <span class="me-2">${serviceOfBService.serviceName}</span>
                                                        <span class="badge 
                                                    ...
                                                    <span class="fw-bold text-end">
                                                        <fmt:formatNumber value="${serviceOfBService.price * bService.quantity}" />đ
                            ...
                            <c:when test="${not empty booking.payments}">
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    ...
                                    <div class="text-end">
                                        <div class="text-danger fw-bold fs-5">
                                            <fmt:formatNumber value="${totalAmount}" />đ
                                        </div>
                                ...
                                <div class="d-flex justify-content-between fw-bold mb-3">
                                    ...
                                    <span class="text-success">
                                        <fmt:formatNumber value="${paidAmount != null ? paidAmount : 0}" />đ
                                    </span>
                                </div>
                                    ...
                                    <div class="collapse" id="paymentInfo">
                                        <div class="text-muted small mt-2">
                                            <c:forEach var="payment" items="${booking.payments}" varStatus="loop">
                                                <c:set var="pmStatus" value="${payment.status}"/>
                                                <div class="card mb-3 border">
                                                    <div class="card-header bg-light d-flex justify-content-between">
                                                        <span><strong>Thanh toán #${loop.count}</strong></span>
                                                        <span class="badge 
                                                            ${pmStatus == 'COMPLETED' ? 'bg-success' :
                                                            pmStatus == 'FAILED' ? 'bg-danger' :
                                                            pmStatus == 'PENDING' ? 'bg-primary' : 'bg-secondary'}">
                                                            ${pmStatus.displayName}
                                                        </span>
                                                    </div>
                                                    <div class="card-body">
                                                        <p><strong>Phương thức:</strong>
                                                            <c:set var="pmType" value="${payment.paymentType}"/>
                                                            <span class="badge 
                                                                ${pmType == 'TRANSFER' ? 'bg-success' :
                                                                pmType == 'CASH' ? 'bg-info' : 'bg-secondary'}">
                                                                ${pmType == 'TRANSFER' ? 'Chuyển khoản' :
                                                                pmType == 'CASH' ? 'Tiền mặt' : 'Chưa xác định'}
                                                            </span>
                                                        </p>
                                ...
                                <c:if test="${status == 'CANCELLED'}">
                                    ...
                                    <div class="d-flex justify-content-between mb-2">
                                        ...
                                        <span class="fw-bold">
                                            <c:choose>
                                                <c:when test="${totalAmount == paidAmount}">
